import { useNavigate, useLocation, useParams } from "react-router-dom";
import { useContext, useState, useRef, useEffect, isValidElement } from "react";
import { DocsContext, AuthContext } from "../../../Conexts/Contexts";

const CreateNewDoc = () => {


  const { docId, docTitle } = useParams();


  const {
    addNewDocToDB,
    saveDoc,
    setDocsUpdated,
    DeleteDoc,
    giveMeThisDoc,
    docs,
    makeDocFavorites
  } = useContext(DocsContext);



  const thisDoc = giveMeThisDoc(parseInt(docId));

  const location = useLocation();

  const { docContent } = location.state || {};

  const navigate = useNavigate();

  const { userId } = useContext(AuthContext);

  const [title, setTitle] = useState(docTitle ? docTitle : "untitled");
  const [content, setContent] = useState("");
  const [isFavorite, setIsFavorite] = useState(thisDoc ? thisDoc['is_favorite'] : false);

  // Create Ref for title input
  const titleRef = useRef(null);

  const handleDocTitleChange = (e) => {
    setTitle(e.target.value);
    console.log(title);
  };

  const handleDocContentChange = (e) => {
    setContent(e.target.value);
    console.log(content);
  };

  const simpleStar = (
    <div className="w-10 h-10">
      <svg
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        stroke="#000000"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          {" "}
          <path
            d="M11.245 4.174C11.4765 3.50808 11.5922 3.17513 11.7634 3.08285C11.9115 3.00298 12.0898 3.00298 12.238 3.08285C12.4091 3.17513 12.5248 3.50808 12.7563 4.174L14.2866 8.57639C14.3525 8.76592 14.3854 8.86068 14.4448 8.93125C14.4972 8.99359 14.5641 9.04218 14.6396 9.07278C14.725 9.10743 14.8253 9.10947 15.0259 9.11356L19.6857 9.20852C20.3906 9.22288 20.743 9.23007 20.8837 9.36432C21.0054 9.48051 21.0605 9.65014 21.0303 9.81569C20.9955 10.007 20.7146 10.2199 20.1528 10.6459L16.4387 13.4616C16.2788 13.5829 16.1989 13.6435 16.1501 13.7217C16.107 13.7909 16.0815 13.8695 16.0757 13.9507C16.0692 14.0427 16.0982 14.1387 16.1563 14.3308L17.506 18.7919C17.7101 19.4667 17.8122 19.8041 17.728 19.9793C17.6551 20.131 17.5108 20.2358 17.344 20.2583C17.1513 20.2842 16.862 20.0829 16.2833 19.6802L12.4576 17.0181C12.2929 16.9035 12.2106 16.8462 12.1211 16.8239C12.042 16.8043 11.9593 16.8043 11.8803 16.8239C11.7908 16.8462 11.7084 16.9035 11.5437 17.0181L7.71805 19.6802C7.13937 20.0829 6.85003 20.2842 6.65733 20.2583C6.49056 20.2358 6.34626 20.131 6.27337 19.9793C6.18915 19.8041 6.29123 19.4667 6.49538 18.7919L7.84503 14.3308C7.90313 14.1387 7.93218 14.0427 7.92564 13.9507C7.91986 13.8695 7.89432 13.7909 7.85123 13.7217C7.80246 13.6435 7.72251 13.5829 7.56262 13.4616L3.84858 10.6459C3.28678 10.2199 3.00588 10.007 2.97101 9.81569C2.94082 9.65014 2.99594 9.48051 3.11767 9.36432C3.25831 9.23007 3.61074 9.22289 4.31559 9.20852L8.9754 9.11356C9.176 9.10947 9.27631 9.10743 9.36177 9.07278C9.43726 9.04218 9.50414 8.99359 9.55657 8.93125C9.61593 8.86068 9.64887 8.76592 9.71475 8.57639L11.245 4.174Z"
            stroke="#000000"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>{" "}
        </g>
      </svg>
    </div>
  );

  const lightStar = (
    <div className="w-10 h-10">
      <svg
        version="1.0"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 64 64"
        enableBackground="new 0 0 64 64"
        xmlSpace="preserve"
        fill="#000000"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          {" "}
          <g>
            {" "}
            <path
              fill="#394240"
              d="M31.998,2.478c0.279,0,0.463,0.509,0.463,0.509l8.806,18.759l20.729,3.167l-14.999,15.38l3.541,21.701 l-18.54-10.254l-18.54,10.254l3.541-21.701L2,24.912l20.729-3.167l8.798-18.743C31.527,3.002,31.719,2.478,31.998,2.478 M31.998,0 c-0.775,0-1.48,0.448-1.811,1.15l-8.815,18.778L1.698,22.935c-0.741,0.113-1.356,0.632-1.595,1.343 c-0.238,0.71-0.059,1.494,0.465,2.031l14.294,14.657L11.484,61.67c-0.124,0.756,0.195,1.517,0.822,1.957 c0.344,0.243,0.747,0.366,1.151,0.366c0.332,0,0.666-0.084,0.968-0.25l17.572-9.719l17.572,9.719 c0.302,0.166,0.636,0.25,0.968,0.25c0.404,0,0.808-0.123,1.151-0.366c0.627-0.44,0.946-1.201,0.822-1.957l-3.378-20.704 l14.294-14.657c0.523-0.537,0.703-1.321,0.465-2.031c-0.238-0.711-0.854-1.229-1.595-1.343l-19.674-3.006L33.809,1.15 C33.479,0.448,32.773,0,31.998,0L31.998,0z"
            ></path>{" "}
            <path
              fill="#ffcc00"
              d="M31.998,2.478c0.279,0,0.463,0.509,0.463,0.509l8.806,18.759l20.729,3.167l-14.999,15.38l3.541,21.701 l-18.54-10.254l-18.54,10.254l3.541-21.701L2,24.912l20.729-3.167l8.798-18.743C31.527,3.002,31.719,2.478,31.998,2.478"
            ></path>{" "}
          </g>{" "}
        </g>
      </svg>
    </div>
  );

  useEffect(() => {
    // Focus on title input when component mounts
    if (titleRef.current) {
      titleRef.current.focus();

      console.log(thisDoc);
    }
    // Reset title and content if creating a new doc
    if (location.pathname === `/${userId}/docs/create-doc`) {
      setTitle("");
      setContent("");
    } else if (docTitle) {
      setTitle(docTitle);
      setContent(docContent);
    }

    


  }, [location.pathname, docTitle, userId]);

  const handleDocSave = async () => {
    console.log(location.pathname);

    if (location.pathname === `/${userId}/docs/create-doc`) {


    
        
      const res = await addNewDocToDB(title === "" ? "untitled": title, content, userId);
      if (res) {
        // Navigate only if save is successful
        alert("Document saved successfully");
        setDocsUpdated((prev) => !prev);
        console.log(res);
        // console.log(res.data.message);
      }
    } else {
      // Handle updating the document logic here
      const res = await saveDoc(docId, title, content, isFavorite);

      if (res) {
        alert("Document updated successfully");
      }

      setDocsUpdated((prev) => !prev);
    }

    // Optionally, clear fields after saving
    setTitle("");
    setContent("");
    setIsFavorite(false)

    navigate(`/${userId}/docs`);
  };

  const handleDocDelete = async () => {
    const res = await DeleteDoc(docId);

    if (res) {
      alert("deleted successfully");
      setDocsUpdated((prev) => !prev);
      navigate(`/${userId}/docs`);
    }
  };

  const toggleFavorite = async () => {
    setIsFavorite((prev) => !prev);
    console.log(thisDoc);
    console.log(typeof docId);

  };

  return (
    <>
      <div className="flex items-center mb-4 gap-4">
        <button onClick={toggleFavorite}>
          {isFavorite ? lightStar : simpleStar}
        </button>

        <input
          type="text"
          value={title}
          ref={titleRef}
          onChange={handleDocTitleChange}
          placeholder="Title"
          className="p-2 flex-grow border rounded-md focus:outline-blue-400 focus:outline-offset-2 bg-gray-50 placeholder:font-semibold"
        />

        {/* {(location.pathname === `/${userId}/docs/create-doc` || location.pathname === `/${userId}/docs/${docId}/${docTitle}`) && (
                    
                )} */}

        <button
          onClick={handleDocSave}
          className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-6 rounded-md inline-flex items-center shadow-md"
        >
          <span>Save</span>
        </button>

        {location.pathname !== `/${userId}/docs/create-doc` && (
          <button
            onClick={handleDocDelete}
            className="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-6 rounded-md inline-flex items-center shadow-md"
          >
            <span>Delete</span>
          </button>
        )}
      </div>

      <div className="flex items-center mb-4">
        <textarea
          rows="18"
          value={content}
          onChange={handleDocContentChange}
          placeholder="Write here..."
          className="p-6 flex-grow border rounded-md focus:outline-blue-400 focus:outline-offset-2 bg-gray-50 placeholder:font-semibold"
        />
      </div>
    </>
  );
};

export default CreateNewDoc;
