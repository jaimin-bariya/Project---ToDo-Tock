  ...
  return (
    <>
      <div className="max-h-96 min-h-64 overflow-y-auto ">
        <p className="mb-2">My Perosnal Docs </p>
        <hr />
        ...
        <div className="p-6">
          ...
          <ul className="space-y-2 grid grid-cols-3 gap-4">
            {docs
              .filter((doc) => doc.is_type_project === false  )
              .map((doc) => (
                <li
                  key={doc.id}
                ...
                >
                  ...
                  <span className="font-bold text-lg mr-4">{doc.projectName === null ? "" : ""} </span>
                  <div className="flex items-center">
